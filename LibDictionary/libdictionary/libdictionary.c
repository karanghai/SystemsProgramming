...
int dictionary_add(dictionary_t *d, const char *key, const char *value)
{
	...
	else 
	{
		...
		if(strcmp(temp->key,key) == 0)
			return KEY_EXISTS;		
		temp->next = malloc(sizeof(dictionary_entry_t));
		temp = temp->next;
		temp->key = key;
		temp->value = value;
...
{	
	...
	char * value1 = key_value+colon+2;
	int x =dictionary_add(d,key1,value1);
	return x;
...
{
	dictionary_entry_t * temp = d->entry;
	dictionary_entry_t * temp1 = temp->next;
	if(temp!=NULL)
	{	
		while(temp1!=NULL)
		{	
